...
- **Price history** is in `card_price_history` (run `supabase/migrations/20260921000000_card_price_history.sql` once). `scripts/import_prices.mjs` adds a row for every card whose price is new or changed. The graph in the marketplace card window (`/api/marketplace/price-history`) draws that as the market reference and adds what copies sold for on the site, taken from completed sales in `store_orders`; current listings are not part of it. Sales are read on the server because those records also hold buyers' contact details.
- **Who can read and change what.** The browser talks to the database with a public key, so row-level rules are the only thing between a visitor and the data (`supabase/migrations/20260921010000_tighten_access.sql`). The browser can read only a short list of `settings` keys (exchange rates, reviews, on/off switches), never orders, hold requests or admin settings; it cannot read email addresses from `profiles`; and it cannot write to `inventory`, `inventory_images`, `orders` or `hold_requests` at all. Those changes go through API routes, which use the service role and therefore have to check the caller themselves (`src/lib/requestAuth.ts`). `/api/orders` returns only the caller's own orders (all for an admin), and `/api/settings` changes need an admin. When adding an API route that uses `supabaseAdmin`, check the caller first.
