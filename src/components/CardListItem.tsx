  ...
  return (
    <div
      ...
      style={{
        ...
        border: isOwned ? "1px solid rgba(52,211,153,0.3)" : "1px solid rgba(99,102,241,0.15)",
        boxShadow: isOwned ? `0 4px 24px rgba(52,211,153,0.15), 0 0 0 0 transparent` : `0 4px 24px rgba(0,0,0,0.4), 0 0 0 0 transparent`,
      }}
      onMouseEnter={(e) => {
        (e.currentTarget as HTMLElement).style.boxShadow = isOwned ? `0 12px 40px rgba(52,211,153,0.25), 0 0 20px ${rarityStyle.glow}` : `0 12px 40px rgba(0,0,0,0.6), 0 0 20px ${rarityStyle.glow}`;
        (e.currentTarget as HTMLElement).style.borderColor = isOwned ? "rgba(52,211,153,0.5)" : `${rarityStyle.text}44`;
      }}
      onMouseLeave={(e) => {
        (e.currentTarget as HTMLElement).style.boxShadow = isOwned ? `0 4px 24px rgba(52,211,153,0.15), 0 0 0 0 transparent` : `0 4px 24px rgba(0,0,0,0.4), 0 0 0 0 transparent`;
        (e.currentTarget as HTMLElement).style.borderColor = isOwned ? "rgba(52,211,153,0.3)" : "rgba(99,102,241,0.15)";
      }}
    >
      {/* Image Area */}
        ...
        >
          ...
          {card.image_path ? (
            <img src={getCardImageUrl(card.image_path)} alt={card.name}
              style={{ position: 'relative', zIndex: 1, maxWidth: '88%', maxHeight: '88%', objectFit: 'contain', borderRadius: 4, filter: isOwned ? 'none' : 'grayscale(100%) opacity(0.7)' }}
            />
          ) : (
            <div style={{ position: 'relative', zIndex: 1, textAlign: 'center', filter: isOwned ? 'none' : 'grayscale(100%) opacity(0.7)' }}>
      ...
      <div className="p-4 flex flex-col flex-grow">
        ...
        <div className="mt-auto pt-3" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
          <button
            ...
            className="w-full text-sm font-bold px-4 py-2 rounded-xl transition-all active:scale-95 flex items-center justify-center gap-2 hover:brightness-125"
            style={{
              background: isOwned ? "rgba(52,211,153,0.15)" : "rgba(255,255,255,0.03)",
              color: isOwned ? "#34d399" : "#9ca3af",
              border: isOwned ? "1px solid rgba(52,211,153,0.4)" : "1px solid rgba(255,255,255,0.1)",
            }}
          >
            {isOwned ? (
              <>
                <span className="text-emerald-400">✓</span> In Collection
              </>
            ) : (
              <>
                <span className="text-gray-500">+</span> Missing
              </>
            )}
          </button>
        </div>
