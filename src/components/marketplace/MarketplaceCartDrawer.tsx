...
export function MarketplaceCartDrawer() {
  ...
  const [handoverMethod, setHandoverMethod] = useState('personal');
  ...
  const [successMsg, setSuccessMsg] = useState<{ text: string; holdRequestId: string } | null>(null);
  ...
  const sellerName = cart[0]?.sellerName || 'Seller';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg({
        text: `Hold request for ${cart.length} card${cart.length === 1 ? '' : 's'} sent to ${sellerName}!`,
        holdRequestId: data.data?.id,
      });
      clearCart();
    } catch (err: any) {
    ...
    <>
      ...
      {!open && (
        ...
        >
          ...
          <span className="text-xs font-black">{cart.length} card{cart.length === 1 ? '' : 's'} · {fmt(total)}</span>
        </button>
      )}
      ...
      {open && (
        ...
        >
          ...
          >
            ...
            {successMsg ? (
              <div className="p-4 rounded-xl bg-emerald-500/15 border border-emerald-500/40 text-emerald-300 text-xs font-semibold space-y-2">
                ...
                {successMsg.holdRequestId && (
                  <a href={`/messages?hold_request_id=${successMsg.holdRequestId}`} className="underline underline-offset-2 hover:text-emerald-200 block">
                    Message the seller →
                  </a>
                )}
              </div>
              ...
              <>
                ...
                <form onSubmit={handleSubmit} className="space-y-3">
                      ...
                      {HANDOVER_METHODS.map((m) => (
                        <button
                          ...
                          className={`p-2 rounded-lg border text-[11px] font-bold transition cursor-pointer ${
                            handoverMethod === m.id ? 'bg-amber-500/15 border-amber-500/60 text-amber-300' : 'border-zinc-800 bg-zinc-950/50 text-zinc-300 hover:bg-zinc-900/60'
                          }`}
                        >
                          {m.label}
                        </button>
                  ...
                  >
                    {submitting ? 'Sending…' : `Request Hold on ${cart.length} Card${cart.length === 1 ? '' : 's'}`}
                  </button>
                </form>
              </>
            )}
